...
public sealed class PlayerCombat
{
    ...
    public const float DodgeInvulnerabilityDuration = 0.2f;
    ...
    {
        if (AttackState != PlayerAttackState.Ready || Stamina < AttackStaminaCost) return false;
    ...
    {
        if (_dodgeCooldownTimer > 0f || IsDodging || Stamina < DodgeStaminaCost) return false;
    ...
    private void TickStamina(float dt)
    {
        ...
        if (_staminaRegenDelayTimer > 0f || Stamina >= MaxStamina) return;
