...
public sealed class RaycastRenderer
{
    ...
    public RaycastRenderer(DungeonMap map, Texture2D wallTexture)
    {
        ...
        unsafe
        {
            ...
            for (int i = 0; i < _wallPixels.Length; i++) _wallPixels[i] = pixels[i];
    ...
    public void Draw(PlayerController player)
    {
        ...
        Raylib.DrawTexturePro(_frameTexture, new Rectangle(0, 0, InternalWidth, InternalHeight), new Rectangle(0, 0, windowW, windowH), Vector2.Zero, 0f, Color.White);
    }

    private void DrawFloor(PlayerController player, int horizon) => DrawHorizontalPlane(player, horizon, Math.Clamp(horizon + 1, 0, InternalHeight), InternalHeight, true);
    private void DrawCeiling(PlayerController player, int horizon) => DrawHorizontalPlane(player, horizon, 0, Math.Clamp(horizon, 0, InternalHeight), false);
    ...
    private void DrawEnemies(PlayerController player, int horizon)
    {
        ...
        foreach (Enemy enemy in _map.Enemies.Where(e => e.IsAlive).OrderByDescending(e => e.DistanceToPlayer))
        {
            ...
            float angleToEnemy = MathF.Atan2(toEnemy.Y, toEnemy.X) - player.Angle;
            angleToEnemy = MathF.Atan2(MathF.Sin(angleToEnemy), MathF.Cos(angleToEnemy));
            if (MathF.Abs(angleToEnemy) > halfFov + 0.2f) continue;

            float screenX = (0.5f + (angleToEnemy / _fov)) * InternalWidth;
            int spriteHeight = (int)((DungeonMap.TileSize / MathF.Max(distance, 0.01f)) * projPlaneDist);
            int spriteWidth = spriteHeight;
            int top = horizon - spriteHeight / 2;
            int left = (int)(screenX - spriteWidth / 2f);

            byte shade = (byte)(255 * Math.Clamp(1f - (distance / _maxRayDistance), 0.25f, 1f));

            for (int sx = 0; sx < spriteWidth; sx++)
            {
                int screenCol = left + sx;
                if (screenCol < 0 || screenCol >= InternalWidth) continue;
                if (distance > _depthBuffer[screenCol]) continue;

                int texX = (int)(sx / (float)spriteWidth * enemy.Texture.Width);
                for (int sy = 0; sy < spriteHeight; sy++)
                {
                    int screenY = top + sy;
                    if (screenY < 0 || screenY >= InternalHeight) continue;
                    int texY = (int)(sy / (float)spriteHeight * enemy.Texture.Height);
                    var sprite = GetSpritePixels(enemy.Texture);
                    Color texel = sprite.Pixels[(texY * sprite.Width) + texX];
                    if (texel.A < 10) continue;
                    _framebuffer[screenY * InternalWidth + screenCol] = Modulate(texel, shade);
                }
            }
        }
    }
    ...
    private (float Distance, float TextureX, bool HitVertical) CastRay(Vector2 origin, float rayAngle) { /* unchanged */
        Vector2 rayDir = new(MathF.Cos(rayAngle), MathF.Sin(rayAngle)); int mapX = (int)(origin.X / DungeonMap.TileSize); int mapY = (int)(origin.Y / DungeonMap.TileSize);
        float deltaDistX = rayDir.X == 0f ? float.MaxValue : MathF.Abs(DungeonMap.TileSize / rayDir.X); float deltaDistY = rayDir.Y == 0f ? float.MaxValue : MathF.Abs(DungeonMap.TileSize / rayDir.Y);
        int stepX = rayDir.X < 0 ? -1 : 1; int stepY = rayDir.Y < 0 ? -1 : 1;
        float sideDistX = rayDir.X < 0 ? (origin.X - mapX * DungeonMap.TileSize) / -rayDir.X : ((mapX + 1) * DungeonMap.TileSize - origin.X) / (rayDir.X == 0f ? 0.0001f : rayDir.X);
        float sideDistY = rayDir.Y < 0 ? (origin.Y - mapY * DungeonMap.TileSize) / -rayDir.Y : ((mapY + 1) * DungeonMap.TileSize - origin.Y) / (rayDir.Y == 0f ? 0.0001f : rayDir.Y);
        bool hitVertical = false; float distance = 0f;
        while (distance < _maxRayDistance) { if (sideDistX < sideDistY) { distance = sideDistX; sideDistX += deltaDistX; mapX += stepX; hitVertical = true; } else { distance = sideDistY; sideDistY += deltaDistY; mapY += stepY; hitVertical = false; } if (_map.IsWallAtGrid(mapX, mapY)) break; }
        Vector2 hitPoint = origin + rayDir * distance; float textureCoord = hitVertical ? hitPoint.Y : hitPoint.X; float textureX = textureCoord % DungeonMap.TileSize; if (textureX < 0f) textureX += DungeonMap.TileSize;
        return (distance, (textureX / DungeonMap.TileSize) * _wallWidth, hitVertical);
    }
    private static Color Modulate(Color c, byte shade) => new((byte)(c.R * shade / 255), (byte)(c.G * shade / 255), (byte)(c.B * shade / 255), (byte)255);
    private static int PositiveMod(int value, int modulus) { int m = value % modulus; return m < 0 ? m + modulus : m; }
    private static int WorldToTileTexel(float worldCoord, int tileSize, int textureSize) { int cellLocal = PositiveMod((int)MathF.Floor(worldCoord), tileSize); return (cellLocal * textureSize) / tileSize; }
    private void DrawCrosshair(){int cx = InternalWidth/2; int cy = InternalHeight/2; DrawLineCPU(cx-8,cy,cx+8,cy,new Color(220,220,220,180)); DrawLineCPU(cx,cy-8,cx,cy+8,new Color(220,220,220,180));}
    private void DrawLineCPU(int x0,int y0,int x1,int y1,Color color){int dx=Math.Abs(x1-x0);int sx=x0<x1?1:-1;int dy=-Math.Abs(y1-y0);int sy=y0<y1?1:-1;int err=dx+dy;while(true){if((uint)x0<InternalWidth&&(uint)y0<InternalHeight){_framebuffer[y0*InternalWidth+x0]=color;}if(x0==x1&&y0==y1)break;int e2=2*err;if(e2>=dy){err+=dy;x0+=sx;}if(e2<=dx){err+=dx;y0+=sy;}}}

    public void DrawMinimap(PlayerController player)
    {
        const int cell = 16; const int offsetX = 16; const int offsetY = 16;
        for (int y = 0; y < _map.Height; y++) for (int x = 0; x < _map.Width; x++) { bool wall = _map.IsWallAtGrid(x,y); Raylib.DrawRectangle(offsetX + x*cell, offsetY + y*cell, cell-1, cell-1, wall ? new Color(84,86,90,220) : new Color(32,36,40,170)); }
        ...
        float px = offsetX + (player.Position.X / DungeonMap.TileSize) * cell; float py = offsetY + (player.Position.Y / DungeonMap.TileSize) * cell;
        Vector2 forward = new(MathF.Cos(player.Angle), MathF.Sin(player.Angle)); Vector2 right = new(-forward.Y, forward.X);
        Vector2 center = new(px,py); Vector2 tip = center + (forward * 8f); Vector2 left = center - (forward * 4f) - (right * 4f); Vector2 rightPoint = center - (forward * 4f) + (right * 4f);
